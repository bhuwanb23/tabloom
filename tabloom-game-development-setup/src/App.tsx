...
function loadMeta(): MetaSave {
  try {
    ...
    if (raw) return { codex: [], act1Complete: false, act2Complete: false, muted: false, ...JSON.parse(raw) };
  } catch {
    /* ignore */
  }
  return { codex: [], act1Complete: false, act2Complete: false, muted: false };
}
...
export default function App() {
  ...
  const onEnd = useCallback((stats: RunStats) => {
    setMeta((m) => {
      ...
      return { ...m, act1Complete: true, act2Complete: true };
    });
    try {
      localStorage.removeItem(RUN_KEY);
    } catch {
  ...
  return (
    <div className="h-full overflow-hidden bg-[#05070a] text-[#e8ecef]">
      {phase === "title" && (
        <TitleScreen
          hasSave={hasSave}
          completed={meta.act2Complete ? 2 : meta.act1Complete ? 1 : 0}
          onBegin={begin}
      ...
      {phase === "end" && endStats && (
        <EndOfAct
          act={2}
          completedActs={[1, ...(meta.act2Complete ? [2] : [])]}
